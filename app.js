	...
	switch(req.user.classCode) {
		case "대학생":
			...
			if (Array.isArray(req.user)) {
				res.render('profile/student.ejs', {user: req.user[0]._doc});
			} else {
				res.render('profile/student.ejs', {user: req.user});
			} break;
		case "대회주최자":
			console.log('이름: '+req.user.name);
			if (Array.isArray(req.user)) {
				res.render('profile/host.ejs', {user: req.user[0]._doc});
			} else {
				res.render('profile/host.ejs', {user: req.user});
			} break;
		case "시상인증자":
			console.log('이름: '+req.user.name);
			if (Array.isArray(req.user)) {
